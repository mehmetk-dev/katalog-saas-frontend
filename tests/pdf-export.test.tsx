...
import { clientCreatePdfExportJob } from '@/lib/hooks/pdf-export-client-api'
...
vi.mock('@/lib/hooks/pdf-export-client-api', () => ({
    clientCreatePdfExportJob: vi.fn().mockResolvedValue({
        job: { id: 'job-1', status: 'queued', progress: 15 },
    }),
    clientGetPdfExportJob: vi.fn(() => new Promise(() => undefined)),
    clientGetPdfExportShareLink: vi.fn(),
    clientCancelPdfExportJob: vi
        .fn()
        .mockResolvedValue({ job: { id: 'job-1', status: 'cancelled' } }),
}))
...
describe('BuilderPageClient PDF Export', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks()
        ;(
            useRouter as unknown as { mockReturnValue: (value: typeof mockRouter) => void }
        ).mockReturnValue(mockRouter)
        ;(
            useUser as unknown as {
                mockReturnValue: (value: {
                    user: { plan: string; exports_remaining: number }
                    canExport: () => boolean
                    refreshUser: () => void
                }) => void
            }
    ...
    it('should open PDF progress flow when download is triggered', async () => {
        ...
        await waitFor(() => {
            expect(clientCreatePdfExportJob).toHaveBeenCalledWith('1', 'standard')
        })
