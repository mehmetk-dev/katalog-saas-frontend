        ...
        <div className="space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2 sm:col-span-2">
                    ...
                    <Input id="name" value={name} onChange={(e) => onNameChange(e.target.value)} required placeholder={t("products.productNamePlaceholder")} className="h-11" />
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        <Input id="sku" value={sku} onChange={(e) => onSkuChange(e.target.value)} placeholder={t("products.skuPlaceholder")} className="h-11" />
            ...
            <div className="space-y-2 pt-2">
                <button type="button" onClick={() => setShowCategories(!showCategories)} className="w-full flex items-center justify-between p-3 rounded-lg border bg-muted/30 hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-2">
                        <FolderPlus className="w-4 h-4 text-violet-600" />
                        <span className="font-medium text-sm">{t("categories.title")}</span>
                        {category.length > 0 && <Badge variant="secondary" className="bg-violet-100 text-violet-700 text-xs">{t("products.selected", { count: category.length })}</Badge>}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 px-1">
                        {category.map((cat, idx) => (
                            <Badge key={idx} variant="secondary" className="pl-2 pr-1 py-0.5 gap-1 text-xs bg-violet-50 text-violet-700 border-violet-100">
                                {cat}
                                <button type="button" onClick={() => onCategoryChange(category.filter((_, i) => i !== idx))} className="ml-0.5 hover:bg-violet-200 rounded-full p-0.5">
                ...
                {showCategories && (
                    <div className="space-y-3 p-3 border rounded-lg bg-background animate-in slide-in-from-top-2 duration-200">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {allCategories.map((cat) => (
                                        <button key={cat} type="button" onClick={() => toggleCategory(cat)} className={cn("px-2.5 py-1 text-xs rounded-full border transition-all", category.includes(cat) ? "bg-violet-600 text-white border-violet-600" : "bg-background hover:bg-violet-50 hover:border-violet-300")}>
                                            {cat}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="flex gap-2">
                            <Input value={categoryInput} onChange={(e) => setCategoryInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && categoryInput.trim()) { e.preventDefault(); addNewCategory() } }} placeholder={t("products.newCategory")} className="h-8 text-sm" />
                            <Button type="button" size="sm" className="h-8 px-3 bg-violet-600 hover:bg-violet-700" onClick={addNewCategory} disabled={!categoryInput.trim()}>
            ...
            <div className="space-y-2 pt-2">
                ...
                <Input id="productUrl" type="url" value={productUrl} onChange={(e) => onProductUrlChange(e.target.value)} placeholder="https://example.com/urun-sayfasi" className="h-10" />
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    ...
                    <Button type="button" variant="ghost" size="sm" className="h-7 text-xs gap-1.5 text-violet-600 hover:text-violet-700 hover:bg-violet-50" onClick={generateMagicDescription}>
                        <Wand2 className="w-3.5 h-3.5" />
                        {t("products.generateAi")}
                    </Button>
                </div>
                <Textarea id="description" value={description} onChange={(e) => onDescriptionChange(e.target.value)} placeholder={t("products.descriptionPlaceholder")} rows={4} className="resize-none" />
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 pt-2 border-t">
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 p-3 border rounded-lg bg-muted/30">
                        ...
                        <Input id="price" type="text" inputMode="decimal" value={price} onChange={(e) => onPriceChange(e.target.value.replace(/[^0-9.,]/g, ""))} placeholder="0.00" className="flex-1 h-12 text-2xl font-bold border-0 bg-transparent focus-visible:ring-0 text-right" />
                    </div>
                </div>
                <div className="space-y-2">
                    <Label htmlFor="stock" className="text-sm font-medium">{t("products.stockCount")}</Label>
                    <div className="flex items-center gap-2 p-3 border rounded-lg bg-muted/30">
                        <Input id="stock" type="number" min="0" value={stock} onChange={(e) => onStockChange(e.target.value)} placeholder="0" className="flex-1 h-12 text-2xl font-bold border-0 bg-transparent focus-visible:ring-0 text-center" />
